...
final class DateTimeInterval {
  ...
  private void checkUnits(){
    ...
    if(fFrom.unitsAllPresent(DateTime.Unit.YEAR, DateTime.Unit.MONTH, DateTime.Unit.DAY, DateTime.Unit.HOUR, DateTime.Unit.MINUTE, DateTime.Unit.SECOND) ){
      success = true;
    }
    else if( fFrom.unitsAllPresent(DateTime.Unit.YEAR, DateTime.Unit.MONTH, DateTime.Unit.DAY) &&  fFrom.unitsAllAbsent(DateTime.Unit.HOUR, DateTime.Unit.MINUTE, DateTime.Unit.SECOND) ){
      success = true;
    }
  ...
  */ 
  ...
  private void handleMonthOverflow(){
    ...
    if( fDay > daysInMonth ){
      ...
      else if (DateTime.DayOverflow.FirstDay == fDayOverflow) {
        fDay = 1;
        stepMonth();
      }
      else if (DateTime.DayOverflow.LastDay == fDayOverflow) {
        fDay = daysInMonth;
      }
      else if (DateTime.DayOverflow.Spillover == fDayOverflow) {
