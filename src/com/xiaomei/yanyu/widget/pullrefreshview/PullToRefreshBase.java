package com.xiaomei.yanyu.widget.pullrefreshview;
...
import com.xiaomei.yanyu.util.EntitySerializable;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	private OnRootViewTouchEventListener listener;
	...
	public final void setOnRefreshDownListener(OnRefreshDownListener listener) {
		mOnRefreshDownListener = listener;
	}
	/**
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if(listener!=null){
			listener.onBeforeRootViewTouch(event);
		}
		...
		switch (action) {
			...
			case MotionEvent.ACTION_DOWN: {
				if (isReadyForPull()) {
					...
					mIsBeingDragged = false;
					 if (mHeaderLayout!=null&&null != mOnRefreshDownListener) {
						 mOnRefreshDownListener.onRefresh();
						 mHeaderLayout.setTimerefreshing();
					 }				
				}
				break;
			}
	...
	protected void setRefreshingInternal(boolean doScroll) {
		...
		if (null != mHeaderLayout && isReadyForPullDown()) {
			mHeaderLayout.refreshing();
		}
		if (null != mFooterLayout && isReadyForPullUp()) {
	...
	protected void setManulRefreshing(boolean doScroll){
	    mState = REFRESHING;
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String loadingLabel = context.getString(R.string.pull_up_refresh_refreshing_label);
		String pullupLabel = context.getString(R.string.pull_up_to_get_more);
		String pullupReleaseLabel = context.getString(R.string.pull_up_to_refresh_release_label);
        String pullTimeLabel = context.getString(R.string.pull_to_refresh_update_tiem)
                + EntitySerializable.SDF.format(new Date());
		...
		if (mMode == MODE_PULL_DOWN_TO_REFRESH || mMode == MODE_BOTH) {
			mHeaderLayout = new LoadingLayout(context, MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel, pullTimeLabel, a);
			addView(mHeaderLayout, 0, new LayoutParams(ViewGroup.LayoutParams.FILL_PARENT,
		...
		if (mMode == MODE_PULL_UP_TO_REFRESH || mMode == MODE_BOTH) {
			mFooterLayout = new LoadingLayout(context, MODE_PULL_UP_TO_REFRESH, pullupReleaseLabel, pullupLabel,
					loadingLabel, "", a);
			mFooterLayout.reset();
	...
	public static interface OnRefreshDownListener {
	...
	/**
	* @ClassName: OnRootViewTouchEventListener
	* @author zhi.hu@net263.com
	*
	 */
	public static interface OnRootViewTouchEventListener{
		public void onBeforeRootViewTouch(MotionEvent event);
	}
	
	public void setOnRootViewTouchEventListener(OnRootViewTouchEventListener listener){
